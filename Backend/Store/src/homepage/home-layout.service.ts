...
export class HomeLayoutService {
  ...
  private readonly homeCache = new Map<string, { expiresAt: number; value: any }>();
  ...
  async listLayouts() {
    return this.prisma.homePageLayout.findMany({ orderBy: [{ is_active: 'desc' }, { updated_at: 'desc' }] });
  }
  ...
  async updateLayout(id: string, dto: UpdateLayoutDto) {
    const existing = await this.prisma.homePageLayout.findUnique({ where: { id } });
    ...
    if (dto.is_active) {
      await this.prisma.$transaction([
        ...
        this.prisma.homePageLayout.update({
          where: { id },
          data: { ...dto, locale: dto.locale ?? existing.locale, is_active: true },
        }),
      ]);
    ...
    const updated = await this.prisma.homePageLayout.update({ where: { id }, data: dto });
    this.invalidateCache();
    return updated;
  }
  ...
  async deleteLayout(id: string, force = false) {
    const layout = await this.prisma.homePageLayout.findUnique({ where: { id } });
    if (!layout) throw new NotFoundException('Layout not found');
    if (layout.is_active && !force) {
      throw new BadRequestException('Cannot delete active layout without force');
    }
  ...
  async listSections(layoutId: string) {
    return this.prisma.homePageSection.findMany({ where: { layout_id: layoutId }, orderBy: { position: 'asc' } });
  }
  ...
  async listItems(sectionId: string) {
    return this.prisma.homePageSectionItem.findMany({ where: { section_id: sectionId }, orderBy: { position: 'asc' } });
  }

  async createItem(sectionId: string, dto: CreateItemDto) {
    const item = await this.prisma.homePageSectionItem.create({ data: { ...dto, section_id: sectionId } });
    this.invalidateCache();
    return item;
  }

  async updateItem(id: string, dto: UpdateItemDto) {
    const item = await this.prisma.homePageSectionItem.update({ where: { id }, data: dto });
    this.invalidateCache();
    return item;
  }
  ...
  async reorderItems(dto: ReorderItemsDto) {
    await this.prisma.$transaction(
      dto.items.map((item) => this.prisma.homePageSectionItem.update({ where: { id: item.id }, data: { position: item.position } })),
    );
    this.invalidateCache();
    return { success: true };
  }
