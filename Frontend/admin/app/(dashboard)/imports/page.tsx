    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-slate-500 mt-2">
          Total control of the supplier API from a single tab: configuration, key visibility, health checks, structured runs, and what actually reached the catalog.
        </p>
      </div>
      ...
      <section className="rounded-2xl border border-slate-200 bg-white p-6">
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div>
            ...
            <p className="mt-1 text-sm text-slate-500">
              Manage the API name, base URL, active status, current key, and connection checks from the Imports tab.
            </p>
          </div>
        ...
        {!canReadConfig ? (
          <p className="mt-4 text-sm text-slate-500">Your account cannot read supplier API configuration.</p>
        ) : configLoading ? (
          <p className="mt-4 text-sm text-slate-500">Loading API configuration...</p>
        ) : (
          <>
            <div className="mt-4 grid grid-cols-1 gap-4 md:grid-cols-4">
              <div className="rounded-2xl border border-slate-200 bg-slate-50 p-4">
                ...
                <p className="mt-2 text-lg font-semibold text-slate-900">{form.display_name || "—"}</p>
              </div>
              <div className="rounded-2xl border border-slate-200 bg-slate-50 p-4">
                <div className="flex items-center gap-2 text-sm text-slate-500">
                ...
                <p className="mt-2 text-lg font-semibold text-slate-900">{configSource || "—"}</p>
              </div>
              <div className="rounded-2xl border border-slate-200 bg-slate-50 p-4">
                <div className="flex items-center gap-2 text-sm text-slate-500">
                  <PlugZap className="h-4 w-4" />
                ...
                <p className="mt-2 text-lg font-semibold text-slate-900">{formatDate(lastHealthcheckAt)}</p>
              </div>
              <div className="rounded-2xl border border-slate-200 bg-slate-50 p-4">
                <div className="flex items-center gap-2 text-sm text-slate-500">
                ...
                <p className="mt-2 text-lg font-semibold text-slate-900 break-all">
                  {showingSecret ? form.api_key || "—" : storedMaskedKey || "No key stored"}
                </p>
              </div>
            </div>
            ...
            <div className="mt-6 grid grid-cols-1 gap-4 lg:grid-cols-2">
              <div className="space-y-4">
                <div>
                  <label className="mb-2 block text-sm font-medium text-slate-700">API display name</label>
                  <input
              ...
              <div className="space-y-4">
                <div>
                  <div className="mb-2 flex items-center justify-between gap-3">
                    ...
                    >
                      {showingSecret ? <EyeOff className="h-4 w-4" /> : <Eye className="h-4 w-4" />}
                      {showingSecret ? "Hide current key" : "Show current key"}
                    </button>
                  </div>
                  <textarea
      ...
      <section className="space-y-4 rounded-2xl border border-slate-200 bg-white p-6">
        <div className="flex flex-wrap items-start justify-between gap-3">
          <div>
            <h2 className="text-lg font-bold text-slate-900">Structured import runs</h2>
            <p className="mt-1 text-sm text-slate-500">
              Inspect what the provider returned, what persisted, and the most recent SKU-level incidents.
            </p>
          </div>
          <button
        ...
        {providerStats && (
          <div className="grid grid-cols-1 gap-4 md:grid-cols-4">
            <div className="rounded-2xl border border-sky-200 bg-sky-50 p-4">
              <p className="text-xs uppercase tracking-wide text-sky-700">Provider received</p>
              <p className="mt-2 text-2xl font-semibold text-sky-900">{providerStats.aggregates.source_items_received}</p>
            </div>
            <div className="rounded-2xl border border-emerald-200 bg-emerald-50 p-4">
              <p className="text-xs uppercase tracking-wide text-emerald-700">Catalog persisted</p>
              <p className="mt-2 text-2xl font-semibold text-emerald-900">{providerStats.aggregates.persisted_count}</p>
            </div>
            <div className="rounded-2xl border border-amber-200 bg-amber-50 p-4">
              <p className="text-xs uppercase tracking-wide text-amber-700">Validation skipped</p>
              <p className="mt-2 text-2xl font-semibold text-amber-900">{providerStats.aggregates.validation_skipped_count}</p>
            </div>
        ...
        {latestRun && (
          <div className="rounded-2xl border border-slate-200 bg-slate-50 p-4">
            <div className="flex items-start gap-3">
              ...
              <div>
                <p className="font-semibold text-slate-900">Latest run interpretation</p>
                <p className="mt-1 text-sm text-slate-700">
                  {formatDelta(latestRun.source_items_received, latestRun.persisted_count)}
                </p>
              </div>
            </div>
          </div>
        ...
        {runs.length === 0 ? (
          <p className="text-sm text-slate-500">No structured runs recorded yet.</p>
        ) : (
          <div className="space-y-3">
            {runs.map((run) => {
                  ...
                  >
                    <div className="space-y-2">
                      <div className="flex flex-wrap items-center gap-2">
                        ...
                        <span className={`rounded-full border px-2.5 py-1 text-xs font-semibold ${STATUS_STYLES[run.status] ?? STATUS_STYLES.RUNNING}`}>
                      ...
                      <p className="text-sm text-slate-700">
                        {formatDelta(run.source_items_received, run.persisted_count)}
                      </p>
                    </div>
                    {isExpanded ? (
                        ...
                        <div className="space-y-4">
                          <div className="grid grid-cols-2 gap-3 md:grid-cols-4 xl:grid-cols-6">
                            <div className="rounded-xl border border-slate-200 p-3">
                              ...
                              <p className="mt-2 font-semibold text-slate-900">{detail.source_items_received}</p>
                            </div>
                            <div className="rounded-xl border border-slate-200 p-3">
                              <p className="text-xs uppercase tracking-wide text-slate-500">Processed</p>
                              <p className="mt-2 font-semibold text-slate-900">{detail.processed_count}</p>
                            </div>
                            <div className="rounded-xl border border-slate-200 p-3">
                              <p className="text-xs uppercase tracking-wide text-slate-500">Persisted</p>
                              <p className="mt-2 font-semibold text-slate-900">{detail.persisted_count}</p>
                            </div>
                            <div className="rounded-xl border border-slate-200 p-3">
                              <p className="text-xs uppercase tracking-wide text-slate-500">Created</p>
                              <p className="mt-2 font-semibold text-slate-900">{detail.created_count}</p>
                            </div>
                            <div className="rounded-xl border border-slate-200 p-3">
                              <p className="text-xs uppercase tracking-wide text-slate-500">Updated</p>
                              <p className="mt-2 font-semibold text-slate-900">{detail.updated_count}</p>
                            </div>
                            <div className="rounded-xl border border-slate-200 p-3">
                              <p className="text-xs uppercase tracking-wide text-slate-500">Errors</p>
                              <p className="mt-2 font-semibold text-slate-900">{detail.error_count}</p>
                            </div>
                          </div>

                          <div className="grid gap-4 lg:grid-cols-2">
                            <div className="rounded-2xl border border-slate-200 p-4">
                              <h3 className="font-semibold text-slate-900">Run metadata</h3>
                              <pre className="mt-3 max-h-56 overflow-auto rounded-xl bg-slate-950 p-3 text-xs text-slate-100">
                                {JSON.stringify(detail.result_meta_json ?? detail.request_meta_json ?? {}, null, 2)}
                              </pre>
                            </div>
                            <div className="rounded-2xl border border-slate-200 p-4">
                              <h3 className="font-semibold text-slate-900">Latest SKU incidents</h3>
                              {errors.length === 0 ? (
                                <p className="mt-3 text-sm text-slate-500">No SKU incidents recorded for this run.</p>
                              ) : (
        ...
        {latestIncidents.length > 0 && (
          <div className="rounded-2xl border border-slate-200 bg-white p-4">
            <h3 className="font-semibold text-slate-900">Latest run incidents</h3>
            <div className="mt-3 space-y-3">
